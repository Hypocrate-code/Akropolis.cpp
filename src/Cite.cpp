...
void Cite::print_hex(Hexagone *hex, int x, int y, strCalc& calc) const {
    ...
    void Cite::add_hex_data(Hexagone *hex, int x, int y, strCalc& calc) const {
        ...
        // Add colored data inside the hexagon - using the correct lines
        // Type goes on line 2 (y-1)
        if (y-1 >= 0 && x-4 >= 0 && x+5 <= calc[y-1].size()) {
            std::string topLine = center_string(colorCode + typeStr + resetCode, 5);
            std::cout << ".."<<topLine << ".."<<std::endl;
            calc[y-1].replace(x-2, 5, topLine);
        }
        
        // Color goes on line 3 (y)
        if (y >= 0 && x-3 >= 0 && x+4 <= calc[y].size()) {
            std::string midLine = center_string(colorCode + colorStr + resetCode, 6 );
            calc[y].replace(x-2, 6, midLine);
        }
...
void Cite::draw_hex_recursive(Hexagone* hex, int x, int y, strCalc& calc, 
    ...
    if (!hex || drawnHexagones.count(hex)) return;
...
static const std::vector<std::pair<int, int>> directionOffsets = {
    {9, -3},   // NE (index 0) - largeur hexagone = 9
    {0, 5},    // S  (index 1) - hauteur hexagone = 5  
    {9, 2},    // SE (index 2) - largeur hexagone = 9
    {-9, 2},   // SO (index 3) - largeur hexagone = 9
    {0, -5},   // N  (index 4) - hauteur hexagone = 5
    {-9, -2}   // NO (index 5) - largeur hexagone = 9
};
...

{
    ...
    for (size_t i = 0; i < tuiles.size(); ++i) {
        ...
        for (size_t j = 0; j < hexagones.size(); ++j) {
            ...
            const char* directionNames[6] = {"NE", "E", "SE", "SO", "O", "NO"};
...
void Cite::afficher() const
{
    ...
    static strCalc calc= {  ".           .",
        ...
        print_hex(tuiles[0]->get_hexagones()[0], -4,0, calc);
...
void Cite::print_hex(Hexagone *hex, int x, int y, strCalc& calc) const {
    ...
    if (x <= 5 || x>=calc[0].size()+11)
        increase_calc_size_H(calc, 11);
    
    if (y <=0-2 || y>=calc.size() + 2)
        increase_calc_size_V(calc, 4);


    //make x y centered coords
    //x = x - calc[0].size()/2;
    //y = y - calc.size()/2;
    ...
    calc[y-2].replace(x-3, 7, "_____");
    calc[y-1].replace(x-4, 9, "/     \\");
    calc[y].replace(x-5, 11,   "/       \\");
    calc[y+1].replace(x-5, 11, "\\       /");
    calc[y+2].replace(x-4, 9, "\\_____/");
    //std::replace(calc[y].begin()+x-1,calc[y].begin()+x+1,"xxx");
//     _______
//    /       \
//   /         \
//   \         /
...
void Cite::increase_calc_size_H(strCalc &calc, uint32_t size) const {
    std::cout << " increase_calc_size_H" << std::endl;
    for (auto& line : calc){
        line = std::string("#", size) + line + std::string("#", size);
    }
}
...
void Cite::increase_calc_size_V(strCalc &calc, uint32_t size) const {
    std::cout << " increase_calc_size_H" << std::endl;
    strCalc c(size, {std::string(" ", calc[0].size())});
    calc.insert(calc.begin(), c.begin(), c.end());
    calc.insert(calc.end(), c.begin(), c.end());
}
*/
